...
class PlgSystemPrivacyconsent extends JPlugin
{
	...
	private function remindExpiringConsents()
	{
		...
		try
		{
			$users = (int) $db->setQuery($query)->loadObjectList();
		}
		...
		foreach ($users as $user)
		{
			...
			try
			{
				...
				$substitutions = array(
					...
					'[URL]'      => JUri::root(),
					'[TOKENURL]' => JRoute::link('site', 'index.php?option=com_privacy&view=extend&confirm_token=' . $token, false, $linkMode),
					'[FORMURL]'  => JRoute::link('site', 'index.php?option=com_privacy&view=extend', false, $linkMode),
					'[TOKEN]'    => $token,
					'\\n'        => "\n",
				);
